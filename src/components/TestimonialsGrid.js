...
export class TestimonialsGrid {
  ...
  render(language) {
    ...
    if (language === 'ko') {
      // Korean mode: pick 4 random, show KO + EN translation pair
      const selected = this.shuffleAndPick(testimonials, 4);
      selected.forEach((testimonial) => {
        const koCard = this.createCard(testimonial, 'ko');
        grid.appendChild(koCard);

        const enCard = this.createCard(testimonial, 'en');
        enCard.classList.add('testimonial-translation');
        grid.appendChild(enCard);
      });
    } else {
      // English mode: show all
      testimonials.forEach((testimonial) => {
        grid.appendChild(this.createCard(testimonial, language));
      });
    }
